...
router.post('/file_upload', function (req, res) {
   ...
   fs.exists("/var/ftp/pub/"+myid,function(exists){
       if(exists)
       {
		...
		pdftk.input(des_file).stamp("/var/ftp/pub/watermark/u=1861627709,1630773149&fm=26&gp=0.jpg").output(des_file).then(buffer => {return res.send('successful');}).catch(err => {
			    console.error(err);
