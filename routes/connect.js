...
router.get('/dload', (req, res) => {
  ...
  res.download('/var/ftp/pub/1/181090916_周辉霆.pdf', 'D:/' + stuid + '_' + id + '_' + name + '_copy.pdf', function (err) {
    if (err) {
      ...
      fs.readFile(req.files[0].path, function (err, data) { // 异步读取文件内容
        ...
        fs.writeFile(des_file, data, function (err) { // des_file是文件名，data，文件数据，异步写入到文件
          if (err) {
            ...
            cp.exec('libreoffice --headless --convert-to pdf '+des_2+' --outdir'+des_file,function(err,stdout,stderr){
              if(err){
                  console.error(err);
              }
