...
const RouterViewImpl = vue.defineComponent({
    ...
    setup(props, { attrs, slots }) {
        const realRoute = vue.inject(routeLocationKey);
        const route = vue.computed(() => props.route || realRoute);
        const depth = vue.inject(viewDepthKey, 0);
        vue.provide(viewDepthKey, depth + 1);
        const matchedRoute = vue.computed(() => route.value.matched[depth]);
        const ViewComponent = vue.computed(() => matchedRoute.value && matchedRoute.value.components[props.name]);
        ...
        return () => {
            ...
            return children
                ? children
                : Component
                    ? vue.h(Component, componentProps)
                    : null;
        };
    },
});
